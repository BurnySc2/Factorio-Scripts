...
function getDimension(jsonObject){
    var dimension = {
        "minX": 0,
        "maxX": 0,
        "minY": 0,
    ...
    for (let i of jsonObject.blueprint.entities){
        //console.log(i)
        dimension.minX = _.min([dimension.minX, i.position.x])
        dimension.maxX = _.max([dimension.maxX, i.position.x])
        dimension.minY = _.min([dimension.minY, i.position.y])
        dimension.maxY = _.max([dimension.maxY, i.position.y])
        //console.log(dimension.minX)
    }
    return {
        "width": dimension.maxX - dimension.minX,
        "height": dimension.maxY - dimension.minY
    }
}
...
function myFunction() {
    ...
    try {
        if (blueprintString.indexOf("404 (Not Found)") === -1){
            jsonObject = decodeBlueprint(blueprintString)
            dimensions = getDimension(jsonObject)     
            t2.value = "Width: {}\nHeight: {}".format(dimensions.width, dimensions.height)
        }
        else {
            t2.value = "Not a valid pastebin!\nIf I am mistaken, please go to\n{}\nand paste the blueprint string instead of the pastebin.".format(getPastebinUrl(inputValue))
        }
