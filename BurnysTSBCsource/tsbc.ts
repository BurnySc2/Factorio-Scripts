...
let blueprinttypeDict: {[key: string]: Set<string>} = {
...
function updateVisibility() {
    ...
    let blueprinttypeElement =  document.getElementById("blueprinttype") as HTMLSelectElement
...
function updateVisibilityBeltsOptions() {
    let beltsEnabledElement = document.getElementById("beltsenabled") as HTMLInputElement
...
function updateVisibilityRequesterChest() {
    ...
    let chestTypeElement =  document.getElementById("chesttype") as HTMLSelectElement
    ...
    for (let i = 1; i <= 12; i++) {
        ...
        let requestItemElement = document.getElementById(requestItemElementName) as HTMLInputElement
    ...
    for (let i = 1; i <= 12; i++) {
        ...
        let requestItemElement = document.getElementById(requestItemElementName) as HTMLInputElement
        let requestAmountElement = document.getElementById(requestAmountElementName) as HTMLInputElement
        if (i <= maxVisible) {
...
function updateVisibilityFilters() {
    ...
    let filtersEnabledElement =  document.getElementById("usefilters") as HTMLInputElement
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        let filterXElement = document.getElementById(filterXElementName) as HTMLInputElement  
...
function updateVisibilitySequential() {
    let sequentialEnabledElement =  document.getElementById("sequentialenabled") as HTMLInputElement
...
function copyToClipboard() {
    ...
    let outputElement = document.getElementById("blueprintoutput") as HTMLTextAreaElement
    let copyclipboardbutton = document.getElementById("copyclipboardbutton") as HTMLButtonElement
...
declare var encode: any;
declare var decode: any;
// javascript download manager: http://danml.com/download.html https://github.com/rndme/download
...
function generateBlueprint() {
    ...
    let blueprinttypeElement =  document.getElementById("blueprinttype") as HTMLSelectElement
    let selectedOption = blueprinttypeElement.selectedOptions[0].value
    var blueprinttypeDict: {[key: string]: Function} = {
    ...
    let sequentialEnabledElement = document.getElementById("sequentialenabled") as HTMLInputElement
    let sequentialAmountElement = document.getElementById("sequentialamount") as HTMLInputElement
...
function parseLoadingStation(trainStopsAmount: number) {
    console.log("Generating loading station")
    let blueprint: Array<any> = []
...
function parseUnloadingStation(trainStopsAmount: number) {
...
function parseFluidLoadingStation(trainStopsAmount: number) {
...
function parseFluidUnloadingStation(trainStopsAmount: number) {
...
function parseStacker(trainStopsAmount: number) {
...
function placeItem(bp: Array<any>, itemName: string, x: number, y: number, direction: number = 0) {
...
function placeTrainStop(bp: Array<any>, itemName: string, x: number, y: number, railSignals: Array<any>, trainStopsAmount: number) {
    // An extra function just to pass the station name and handle enabled-condition
    let stationNameElement = document.getElementById("stationname") as HTMLInputElement
    let stationName = stationNameElement.value

    let stationEnabledCondition = (document.getElementById("trainstopconditionenabled") as HTMLInputElement).checked
    let greenWire = (document.getElementById("greenwire") as HTMLInputElement).checked
    let redWire = (document.getElementById("redwire") as HTMLInputElement).checked
...
function placeDecider(bp: Array<any>, itemName: string, x: number, y: number, direction: number) {    
    let stationEnabledCondition = (document.getElementById("trainstopconditionenabled") as HTMLInputElement).checked
    let greenWire = (document.getElementById("greenwire") as HTMLInputElement).checked
    let redWire = (document.getElementById("redwire") as HTMLInputElement).checked
    let willPlaceDecider = stationEnabledCondition && (greenWire || redWire)
    ...
    let comparatorType = (document.getElementById("trainstopconditioncomparator") as HTMLSelectElement).selectedOptions[0].value
    let trainStopConditionAmount = parseFloat((document.getElementById("trainstopconditionamount") as HTMLInputElement).value)

    let trainStop: any = bp[bp.length - 1]
    let newEntry: any = {
        "entity_number": bp.length + 1,
...
function connectWireWithDecider(bp: Array<any>, rightChests: Array<any>, leftChests: Array<any>, trainStopIndex: number) { 
    let stationEnabledCondition = (document.getElementById("trainstopconditionenabled") as HTMLInputElement).checked
    let greenWire = (document.getElementById("greenwire") as HTMLInputElement).checked
    let redWire = (document.getElementById("redwire") as HTMLInputElement).checked
    let willPlaceDecider = stationEnabledCondition && (greenWire || redWire)
    ...
    let polesLeft: Array<any> = []
    let polesRight: Array<any> = []
    let deciders: Array<any> = []
    bp.forEach(entity => {
    ...
    let previousPole: any = undefined
...
function placeInserter(bp: Array<any>, itemName: string, x: number, y: number, direction: number, filters: Array<Object>) {    
    // Inserts a new entity into blueprint
    let newEntry: any = {}
    newEntry = {
        "entity_number": bp.length+1,
...
function getRequesterChestRequester(itemName: string, itemAmount: number, index: number = 1): Object {
    return {
        "index": index,
        "name": itemName,
...
function placeRefuelChest(bp: Array<Object>, itemName: string, x: number, y: number) { 
    let fuelTypeElement = document.getElementById("fueltype") as HTMLSelectElement
    let fuelTypeAmountElement = document.getElementById("refillamount") as HTMLInputElement
...
function placeChest(bp: Array<Object>, itemName: string, x: number, y: number, requests: Array<Object>, limit: number = -1, greenWire: boolean = false, redWire: boolean = false, orientation: number = 0) {    
    // Inserts a new entity into blueprint
    let newEntry: any = {
        "entity_number": bp.length+1,
        "name": itemName,
...
function getTrainLength(correctOddValue: boolean = false): number {
	const locomotive_length = 6
	const cargo_length = 6
	const space_between_trains = 1
    let locos = parseFloat((document.getElementById("amountlocomotives") as HTMLInputElement).value)
    let cargos = parseFloat((document.getElementById("amountcargowagons") as HTMLInputElement).value)
    let doubleHeaded = (document.getElementById("traintype") as HTMLInputElement).checked
...
function getYoffset(trainStopIndex: number): number {
    let doubleHeaded = (document.getElementById("traintype") as HTMLInputElement).checked
    let doubleHeadedSubtract = doubleHeaded ? -2 : 0
    return trainStopIndex * (getTrainLength(true) + 6 + doubleHeadedSubtract)
}

function shouldPlaceRightSide(): boolean {  
    let stationTypeElement =  document.getElementById("blueprinttype") as HTMLSelectElement
    let stationType = stationTypeElement.selectedOptions[0].value  
    let sideUsed = (document.getElementById("sidetobeused") as HTMLSelectElement).selectedOptions[0].value
    let fluidSideUsed = (document.getElementById("fluidsidetobeused") as HTMLSelectElement).selectedOptions[0].value
...
function shouldPlaceLeftSide(): boolean {    
    let stationTypeElement =  document.getElementById("blueprinttype") as HTMLSelectElement
    let stationType = stationTypeElement.selectedOptions[0].value
    let sideUsed = (document.getElementById("sidetobeused") as HTMLSelectElement).selectedOptions[0].value
    let fluidSideUsed = (document.getElementById("fluidsidetobeused") as HTMLSelectElement).selectedOptions[0].value
...
function createStationTracks(bp: Array<any>, trainStopIndex: number, trainStopsAmount: number) {
    ...
    let lampnearpoles = (document.getElementById("lampnearpoles") as HTMLInputElement).checked
    let placeRightSide = shouldPlaceRightSide()
    let placeLeftSide = shouldPlaceLeftSide()
...
function createRefuelChest(bp: Array<any>, trainStopIndex: number, trainStopsAmount: number) {
    let refillBoolean = (document.getElementById("refill") as HTMLInputElement).checked
    let offsetY = getYoffset(trainStopIndex)
    const locomotive_length = 6
	const cargo_length = 6
	const space_between_trains = 1
...
function createChestsInserters(bp: Array<any>, trainStopIndex: number, loading: boolean = true) {
    const locomotive_length = 6
	const cargo_length = 6
	const space_between_trains = 1
    let offsetY = getYoffset(trainStopIndex)
    ...
    let inserterType = (document.getElementById("insertertype") as HTMLSelectElement).selectedOptions[0].value
    let useFiltersElement = document.getElementById("usefilters") as HTMLInputElement
    let useFilters = useFiltersElement.checked
    let filterInserters: {[key: string]: string} = {
    ...
    let beltsEnabled = (document.getElementById("beltsenabled") as HTMLInputElement).checked
    let greenWire = (document.getElementById("greenwire") as HTMLInputElement).checked
    let redWire = (document.getElementById("redwire") as HTMLInputElement).checked
    let chestType = (document.getElementById("chesttype") as HTMLSelectElement).selectedOptions[0].value
    let chestlimit = parseFloat((document.getElementById("chestlimit") as HTMLInputElement).value)
    let trainLength = getTrainLength()
    ...
    if (useFilters) {
        for (let i = 1; i <= 5; i++) {
            let thisFilterElement = document.getElementById("filter" + i.toString()) as HTMLInputElement
    ...
    let rightChests: Array<any> = []
    let leftChests: Array<any> = []
...
function createBeltLayout(bp: Array<any>, trainStopIndex: number, trainStopsAmount: number, loading: boolean = true) {
    let beltsEnabledElement = document.getElementById("beltsenabled") as HTMLInputElement
    let beltsEnabled = beltsEnabledElement.checked
    if (! beltsEnabled) {return}
    let offsetY = getYoffset(trainStopIndex)
    const locomotive_length = 6
	const cargo_length = 6
	const space_between_trains = 1
    let beltTypeElement = document.getElementById("belttype") as HTMLSelectElement
    ...
    let doubleHeaded = (document.getElementById("traintype") as HTMLInputElement).checked
    let doubleHeadedInt = doubleHeaded ? 1 : 0
    let splitterTypes: {[key: string]: string} = {
    ...
    let splitterOutput: Array<Array<number>> = []    
...
function compare(a: Array<number>, b: Array<number>): number {
...
function getTargetYLine(splitterOutput: Array<Array<number>>): number {
    let beltFlowDirectionElement = document.getElementById("beltflowdirection") as HTMLSelectElement
    let beltFlowDirection = beltFlowDirectionElement.value
    // let trainLength = getTrainLength()
    // let middleOfTrain = trainLength / 2
...
function getBeltPath(startLocation: Array<number>, targetX: number, targetY: number, loading: boolean, beltFlowDirection: string): Array<Array<number>> {
    // let beltFlowDirectionElement = document.getElementById("beltflowdirection") as HTMLSelectElement
    // let beltFlowDirection = beltFlowDirectionElement.value
...
function createBeltFlow(bp: Array<any>, splitterOutput: Array<Array<number>>, trainStopIndex: number, trainStopsAmount: number, loading: boolean = true) {
    let beltFlowDirectionElement = document.getElementById("beltflowdirection") as HTMLSelectElement
    let beltFlowDirection = beltFlowDirectionElement.value
    let beltTypeElement = document.getElementById("belttype") as HTMLSelectElement
    let beltType = beltTypeElement.value
    // let sideUsedElement = document.getElementById("sidetobeused") as HTMLSelectElement
    // let sideUsed = sideUsedElement.value
    let sequentialBeltsGoAllTheWayElement = document.getElementById("sequentialbeltsalltheway") as HTMLInputElement
...
function compareY(a: any, b: any): number {
...
function connectTwoItemsWithWire(item1: any, item2: any, wireColor: string) {
...
function addGreenWire(rightChests: Array<any>, leftChests: Array<any>) {
    let greenWire = (document.getElementById("greenwire") as HTMLInputElement).checked
    if (! greenWire) {return}
    
    let greenWireSidesElement = document.getElementById("greenwiresides") as HTMLInputElement
...
function addRedWire(rightChests: Array<any>, leftChests: Array<any>) {
    let redWire = (document.getElementById("redwire") as HTMLInputElement).checked
    if (! redWire) {return}
    
    let redWireSidesElement = document.getElementById("redwiresides") as HTMLInputElement
...
function createPumpsAndStorageTanks(bp: Array<any>, trainStopIndex: number, loading: boolean = true) {
    const locomotive_length = 6
	const cargo_length = 6
	const space_between_trains = 1
    let locos = parseFloat((document.getElementById("amountlocomotives") as HTMLInputElement).value)
    ...
    let sideUsed = (document.getElementById("fluidsidetobeused") as HTMLSelectElement).selectedOptions[0].value
    let connectTanks = (document.getElementById("fluidconnecttanks") as HTMLInputElement).checked
...
function createStacker(bp: Array<any>) {
    let lanesElement = document.getElementById("stackerparallel") as HTMLInputElement
    let diagonalElement = document.getElementById("stackerdiagonal") as HTMLInputElement
    let leftRightElement = document.getElementById("stackerleftright") as HTMLInputElement
